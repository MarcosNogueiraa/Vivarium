...
- **Revelação suspense pra Épico+ do tanque (08/08/2026):** `CollectCelebration` (só `variant="tank"`, score ≥ 9.8 — mesmo corte de `BANDS`/§5) revela os atributos **um de cada vez** (900ms entre passos) antes do peixe/raridade em si: corpo/brilho → cauda → dorsal → peitoral, com o peixe borrado (`filter: blur`) e a raridade escondida atrás de "???" até o fim. Clicar a qualquer momento (`useSuspenseReveal`, hook local do componente) pula direto pro resultado completo — nunca força espera passiva sem escape, mesma filosofia do rush de fila/gestação (§8.11), só que aqui é um atalho de UI, não um sink de premium. Não se aplica a Comum/Incomum/Raro (frequentes demais, o suspense viraria fricção) nem ao Ninho (`variant="breeding"`, que já tem seu próprio ritmo — pais, despedida — e sempre mostra a tela na hora, raro ou não).
