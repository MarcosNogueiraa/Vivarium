...
- **Revelação suspense pra coleta de peixe épico+ — ideia do usuário (08/08/2026), não implementada:** hoje `CollectCelebration` mostra o peixe raro+ já pronto, tudo de uma vez (CLAUDE.md 8.8 documenta o `variant="breeding"` da mesma tela). Ideia: pra Épico/Lendário (score ≥ 9.8, `BANDS`/§5), revelar os atributos **um de cada vez**, devagar (corpo/brilho → cauda → dorsal → peitoral → score final), tipo abertura de pacote de card game — constrói suspense proporcional à raridade em vez de entregar tudo de uma vez. O jogador poderia **clicar pra acelerar** a revelação em vez de esperar passivamente (mesma filosofia de "nunca forçar espera sem escape" já usada no rush de fila/gestação, §8.11 — só que aqui não custaria premium, é só um atalho de UI, não um sink). Reaproveitaria: `rarityBreakdownOf`/`bredRarityBreakdown` (já calcula a lista ordenada de atributos que compõem o score, usada no detalhe do peixe) como a sequência de revelação; `CollectCelebration` como o componente que ganharia o modo. Não vale a pena pra Comum/Incomum/Raro (frequentes demais, o suspense viraria fricção) — só épico+ (raro o bastante pra merecer o momento). Nada disso está implementado; fica registrado aqui pra quando for retomado.
