...
- **Mais fontes de XP (20/08/2026, pedido do usuário: "quero mais formas" de subir de nível)** — todas por CONTAGEM de ação, não por valor (preço/venda), pra não virar disfarce de "quem gasta mais sobe mais rápido": `DailyRewardClaimXp=15` (resgatar a recompensa diária), `VendorSaleXp=3` (vender ao NPC), `MarketSaleXp=10` (vendedor, quando a listagem é comprada por outro jogador), `ItemPurchaseXp=5` (comprar item da Loja, SOFT ou PREMIUM). Valores iniciais em `LevelConfig.cs`, a calibrar com uso real, mesmo espírito do resto da economia.
- **Marco de raridade (bônus único, não repetível):** primeira vez que o jogador COLETA um peixe de cada banda Raro+ (`RarityMilestoneXp`: Raro=50, Épico=150, Lendário=400 — Comum/Incomum de fora, universais demais pra marco) via `GameService.AwardRarityMilestoneXpAsync`, chamado nos 3 pontos onde um peixe entra na posse do jogador por COLETA (fila manual, fila automática VIP, filhote do Ninho) — não em compra/transferência. Idempotente: rastreado em `User.RarityBandMilestoneFlags` (bitmask, migration `AddRarityBandMilestoneFlags`). Classificação de banda promovida pra `Vivarium.Core.Gameplay.RarityBands` (fonte única agora — `MarketService.BandNameOf` virou um wrapper fino em cima dela, mesmos cortes de sempre, ver §5).
...
- **Avatar = peixe escolhido manualmente pelo jogador** entre os que já possui (`User.AvatarCreatureInstanceId`, nullable FK) — não é upload nem auto-atualização (mais barato de construir, dá controle ao jogador). Só exige posse (`OwnerId == userId`), não precisa estar no tanque. Endpoint `PUT /api/account/avatar` (`AccountService.SetAvatarAsync`); aparece no Perfil (`ProfileModal.jsx`, seletor em grid, **ordenado por `rarityScore` decrescente** — pedido do usuário, 18/08/2026, facilita achar o peixe mais raro sem rolar a lista) e no botão de conta (`AccountMenu.jsx`, substitui o 👤 fixo).
- **Fallback no Ranking (18/08/2026, pedido do usuário):** quem não escolheu avatar mostra o peixe de MAIOR `rarityScore` que possui (não precisa estar no tanque) — só no Ranking, pra todo mundo aparecer com um ícone, não só quem configurou manualmente; `/me`/`AccountMenu` continuam sem fallback (avatar `null` = ícone padrão 👤). `LeaderboardService.AttachLevelsAndAvatarsAsync` resolve via subquery correlacionada por usuário (`OrderByDescending(RarityScore).First()`) — GroupBy+First não traduz de forma confiável no provedor SQLite (achado rodando os testes), a subquery correlacionada é o padrão que funciona nos dois provedores.
