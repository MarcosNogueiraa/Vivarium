...
> - Lendário: 16.60+
>
> **15/08/2026, dois ajustes no mesmo dia.** 1ª tentativa: o corte inicial dava Raro 1,00%/Épico 0,10% (proporção 10:1) — usuário relatou (e confirmei com dados reais de produção via `Vivarium.AdminReset -- band-distribution`) que Raro parecia comum demais e Épico raro demais na prática. Pedido explícito: "diminuir o piso dos épicos". Só o corte Raro/Épico moveu (14,85→13,27) — Raro caiu pra 0,62%, Épico subiu pra 0,48%. **2ª tentativa (final):** usuário achou os dois quase do mesmo tamanho depois do ajuste acima e pediu especificamente Raro de volta perto de 1,00% com Épico em ~0,30% — como isso não cabe só mexendo na fronteira Raro/Épico (encolher Raro de novo pra crescer o Épico desfaria o ajuste), Incomum doou os 0,20 pontos percentuais que faltavam (12,24%→12,04%, mudança pequena o bastante pra não ser perceptível):
> - Comum: score < 5.45
> - Incomum: 5.45–12.04
> - Raro: 12.04–13.78
> - Épico: 13.78–16.60
...
- **Sinergia por cor de cauda (29/07):** ~~N peixes com a mesma cor de cauda no tanque → cada um multiplica a renda por `1 + SynergyPerMatch·(N−1)` com teto `SynergyMaxBonus` (0.15 / +80%). Ex.: 5 de cauda azul → +60% cada.~~ Cria demanda por peixes específicos no mercado ("montar tanque temático") = uso inteligente das moedas. `GameService` deriva a cor via `TraitGenerator`; `IncomeCalculator.FishIncome`/`SynergyMultiplier`. O cliente exibe a sinergia no tanque (`generator.js: synergyMultiplier`).
  - **v2 (14/08/2026) — sinergia por PARTE.** Usuário notou que empilhar muitos peixes da mesma cor de CAUDA sozinha já batia o teto de +80% fácil demais. Passou a ser calculada por parte (cauda/dorsal/peitoral) **separadamente** — cada uma soma um bônus independente (não precisa as 3 baterem no mesmo grupo de peixes): `bonus(N) = N<2 ? 0 : min(SynergyMaxBonus, SynergyBaseBonus + SynergyPerExtraMatch·(N−2))`, com `SynergyBaseBonus=0.075`, `SynergyPerExtraMatch=0.025`, `SynergyMaxBonus=0.15` — pior caso (3 partes batendo o próprio teto) caiu de +80% (1 parte) pra +45% (3 partes), teoricamente "mais raro" por exigir 3 grupos simultâneos.
  - **v3 (15/08/2026) — o "mais raro" da v2 não era raro na prática.** Investigado com dados reais de produção (`Vivarium.AdminReset -- tank-income <email>`, novo comando diagnóstico) na conta do usuário: com só 5 peixes da mesma cor numa parte já saturando o teto (`0.075+0.025×3=0.15`) e poucas cores possíveis (paleta de 8), um tanque real de 15 peixes tinha **10 batendo o teto nas 3 partes ao mesmo tempo**, rendendo **+40,4%** sobre o base no tanque inteiro — a suposição da v2 de que isso seria raro não se confirmou. Corrigido nas DUAS causas: teto por parte caiu 15%→**10%** (pior caso 3 partes: 45%→**30%**) E o N necessário pra saturar subiu de 5→**9** peixes (`SynergyBaseBonus` 0.075→**0.03**, `SynergyPerExtraMatch` 0.025→**0.01**) — agora exige um tanque bem mais dedicado/temático (quase um monotanque por parte) pra alcançar o teto. Revalidado na mesma conta: uplift total caiu de +40,4%→**+19,4%**, ninguém mais bate o teto sozinho (máximo observado +22%). Sem migration — `coinsPerHour` nunca é persistido por peixe, sempre recalculado ao vivo do `TickConfig` a cada tick (mesmo princípio já documentado nos ajustes anteriores de `IncomeCalculator`).
