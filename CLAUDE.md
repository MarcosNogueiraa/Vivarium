...
- **Degradação escala com peixes (29/07, curva mais agressiva em 07/08):** `base·(1 + DegradationPerFishFactor·nºpeixes)` — k **0.10→0.30** (07/08/2026, a pedido do usuário: fazer o auto-filtro de 500 soft "se pagar" num prazo razoável). Payback do auto-filtro (supondo o jogador filtrar ao bater o patamar de 80%, `IncomeWaterPlateau`) caiu de ~9-12 dias pra **~3,5-7,3 dias** dependendo do tamanho do tanque (3/5/10 peixes). Só o fator por peixe mudou — tanque vazio/começo de jogo não fica mais punitivo, o efeito cresce só com o tanque estabelecido. Revalidado com `Vivarium.Simulation simulate`: carteira dos 3 perfis continua oscilando mas crescendo em 120 dias, com bem mais filtros comprados (esperado). *Nota de sustentabilidade original ainda vale:* isto é sabor/consequência de descuido, **não** o balanceador principal — a renda cresce com raridade (exponencial) e a manutenção com contagem (linear), então num tanque rico o upkeep continua pequeno relativo ao bruto. A sustentabilidade de longo prazo depende dos **ralos de progressão** (aquários em tiers — Fase B; breeding — 8.8), não da manutenção.
...
- 💡 **Ideia futura (não decidida):** degradação da água ponderada por raridade — hoje cada peixe soma sempre a mesma fatia fixa (0,9/h desde o ajuste de 07/08 — era 0,3/h com k=0.10) na degradação, não importa o quanto rende. Ideia discutida: pesar por `score/5` (linear, não pela renda exponencial, pra não punir demais tanques de lendário) — comum continua em ~0,9/h, lendário (~15) sobe pra ~2,7/h. Não implementado ainda, recalibrar via `Vivarium.Simulation` antes de mudar `TickConfig.DegradationPerFishFactor`.
