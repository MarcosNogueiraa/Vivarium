...
- **Gestação escala com a raridade combinada dos pais:** `BreedingCalculator.GestationHours(scoreA, scoreB) = BaseGestationHours · exp(GestationGrowth · (scoreA+scoreB − GestationRefScore))`, clamp `[12h, 240h]` (31/07/2026: base 8→24h, piso 4→12h — ritmo anti-rush, ver §8.11). 2 comuns 24h, 2 raros ~49h, 2 épicos ~101h, 2 lendários e lendários máx no teto de 240h (10 dias).
...
- **Gestação mais lenta:** `BreedingDefaults.BaseGestationHours` 8→**24** (3x — até o casal mais comum agora leva o dia inteiro, não só algumas horas), `MinGestationHours` 4→**12**. O crescimento exponencial pela raridade (`GestationGrowth`) não mudou — só o piso, então a curva relativa entre raridades ficou igual, só 3x mais lenta em todo o espectro. Validado via `Vivarium.Simulation breed`: 2 comuns 24h, 2 raros ~49h, 2 épicos ~101h, 2 lendários no teto de 240h (10 dias).
