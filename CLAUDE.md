...
- **Hook do cascudo (não implementado):** comentário em `GameService.ApplyTickAsync` documentando que o bônus de limpeza passiva de uma futura criatura "cascudo" entra somado a `FilterCapacity` (ou como multiplicador extra no `filterFactor`) — mesma fórmula, mais um termo, sem estrutura nova.
