...
- **Custo de transição entre faixas (08/08/2026, a pedido do usuário — a progressão "estava linear demais"):** `CapacityBand.TransitionCost` — um custo bem acima da curva suave, cobrado só na compra específica que CRUZA o teto de uma faixa (ex: capacidade 5→6 entra no Aquário Grande). `CapacityBands.PriceForUpgrade(currentCapacity)` centraliza a decisão (curva suave normal vs. `TransitionCost` da faixa de destino) — usado tanto por `ItemService.CurrentPrice` quanto por `Vivarium.Simulation`. Valores calibrados via `Vivarium.Simulation economy`/`simulate`: Aquário→Grande **4000 soft** (~10x o último degrau suave do Aquário), Grande→Master **12000 soft** (~19x o último degrau suave do Grande) — ordem de grandeza acima de propósito, pra ser um "gate" de verdade, não mais um passo incremental.
  ...
  - **Frontend:** `StoreView.jsx` mostra um `ConfirmModal` específico ("Trocar de aquário") antes de cobrar o `TransitionCost` — evita que o jogador clique "Comprar" achando que é só mais um upgrade normal e leve um susto com o preço. Gatilho: capacidade atual bate um dos limites de faixa (`TRANSITION_CAPACITIES = [5, 10]`, espelha `CapacityBands.All` — só os limites, não a tabela inteira).
