...
const METRICS = [
  { key: "rarity", label: "Raridade total", suffix: "", format: (v) => v.toFixed(1) },
  { key: "income", label: "Renda por hora", suffix: "/h", format: (v) => v.toFixed(1) },
];
    ...
    return (
      <div className="ranking-spectator">
        ...
        <div className="spectator-stats">
          <span className="stat-chip">🏆 {Number(spectator.rarityTotal).toFixed(1)} <small>raridade total</small></span>
        ...
        <div className="leaderboard glass">
          {data.entries.map((e) => (
            <div key={e.rank} className={`leaderboard-row${e.isSelf ? " is-self" : ""}`}>
              ...
              <span className="value mono"><Coin />{active.format(Number(e.value))}<small>{active.suffix}</small></span>
          ...
          {data.selfOutsideTop && (
            <div className="leaderboard-row is-self leaderboard-self-outside">
              ...
              <span className="value mono"><Coin />{active.format(Number(data.selfOutsideTop.value))}<small>{active.suffix}</small></span>
            </div>
          )}
        </div>
      )}
