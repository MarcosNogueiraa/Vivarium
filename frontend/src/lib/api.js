...
export const api = {
  ...
  adminGrantPremiumAll: (amount) => request("POST", "/api/admin/grant-premium-all", { amount }),
  adminAdjustWallet: (username, currencyCode, mode, amount) =>
    request("POST", "/api/admin/wallet", { username, currencyCode, mode, amount }),
  ...
  adminSendInboxMessage: (title, body, audience, usernames, rewardCurrencyCode, rewardCurrencyAmount, rewardEggKeys) =>
    request("POST", "/api/admin/inbox/send", { title, body, audience, usernames, rewardCurrencyCode, rewardCurrencyAmount, rewardEggKeys }),
};
